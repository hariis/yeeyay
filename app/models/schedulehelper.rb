...
def set_remind_once_fields
    ...
    case (self.freq_type)
		...
		when 0,12  #Not Scheduled
				self.start_dt_tm = Time.at(Date.parse("1970-01-01")).at_beginning_of_day.utc
				self.freq_interval = 0
				self.end_dt_tm = self.start_dt_tm
				
		when 1    #Later Today
				self.start_dt_tm = Time.now.at_beginning_of_day.utc  #00:00
				self.freq_interval = 1
				self.end_dt_tm = Time.now.at_beginning_of_day.tomorrow.ago(60).utc #23:59
		
		when 11    #Pick a date
                                self.start_dt_tm = Time.at(start_dt_tm).at_beginning_of_day.utc
				self.freq_interval = 1
				self.end_dt_tm = Time.at(start_dt_tm).at_beginning_of_day.tomorrow.ago(60).utc #23:59
				
		when 2   #Tomorrow
				self.start_dt_tm = Time.now.tomorrow.at_beginning_of_day.utc
				self.freq_interval = 1
				self.end_dt_tm = Time.now.tomorrow.at_beginning_of_day.tomorrow.ago(60).utc #23:59
				
		when 3   #This Friday
				  
				start_dt = Time.now.to_date
				diff = 5 - start_dt.wday
                                case diff
                                when 0 #On a friday - Move to next friday
                                    start_dt = Time.now.to_date + 7
                                when -1 # on a Saturday
                                    start_dt = Time.now.to_date + 6
                                when 1,2,3,4,5
                                    start_dt = Time.now.to_date + diff
                                end                       
                                self.start_dt_tm = Time.at(start_dt).at_beginning_of_day.utc
				self.end_dt_tm = Time.at(start_dt).at_beginning_of_day.tomorrow.ago(60).utc #23:59
		...
		when 5   #Next Week - Move to Monday of next week
				...
				diff =  1 - start_dt.wday
                                start_dt = Time.now.to_date + diff + 7
...
def self.GetDatesFor(entry)
      @dates =[]
      start_dt = Time.zone.utc_to_local(entry.start_dt_tm).to_date
      end_dt = Time.zone.utc_to_local(entry.end_dt_tm).to_date
  ...
  def self.GetDatesUpToToday(entry)
      ...
      start_dt = Time.zone.utc_to_local(entry.start_dt_tm).to_date
      end_dt = Time.now.utc <= entry.end_dt_tm ? Time.now.to_date : Time.zone.utc_to_local(entry.end_dt_tm).to_date
      case (entry.freq_type)
  ...
  def self.IsEntryDueOn(entry,anydate)
      ...
      case (entry.freq_type)
                ...
                when 0,1,2,3,4,5,11,12    #Not scheduled, Later Today, Tomorrow, This Friday, This Sat, Next week
                        ...
                        #entry_start_dt = Time.zone.utc_to_local(entry.start_dt_tm).to_date  
                        entry_start_dt = entry.start_dt_tm.to_date
                ...
                when 6    #Daily
                          ...
                          #Step 2: Figure out the Start Date										
                          if entry.freq_interval != 1 then                           
                             #actualstartDate = Time.zone.utc_to_local(entry.start_dt_tm).to_date 
                             actualstartDate = entry.start_dt_tm.to_date                              
                          else
                            actualstartDate = start_dt
                          end
